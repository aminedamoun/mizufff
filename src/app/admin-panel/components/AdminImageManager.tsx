...
export default function AdminImageManager() {
  ...
  const toggleIntro = async (id: string) => {
    const image = images.find((img) => img.id === id);
    if (!image) return;

    try {
      await imageService.updateImage(id, { isIntro: !image.isIntro });
      setImages((prev) => prev.map((img) => (img.id === id ? { ...img, isIntro: !img.isIntro } : img)));
    } catch (err: any) {
      setError(err.message || 'Failed to update image');
    }
  };
    ...
    <div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 px-6">
          <div className="bg-background border border-white/10 p-8 max-w-md w-full">
            ...
            <div className="flex gap-3">
              <button onClick={() => deleteImage(deleteConfirm)} className="btn-red py-2 px-4 text-xs">
                Delete
              </button>
      ...
      {showAddImage && (
        <div className="border border-primary/20 bg-primary/5 p-6 mb-8">
          ...
          <div className="flex gap-3">
            <button className="btn-red py-2 px-4 text-xs" onClick={addImage}>
              Save Image
            </button>
            <button className="btn-ghost py-2 px-4 text-xs" onClick={() => setShowAddImage(false)}>
              Cancel
            </button>
          </div>
        </div>
